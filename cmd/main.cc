...
bool compile_source(const std::string& src_abs_path,
                    const std::string& cache_path) {
  std::string tmp_path = cache_path + ".tmp." + std::to_string(getpid());
  int rc = forge::proc::RunWait(
      {"c++", "-std=c++17", src_abs_path, "-o", tmp_path});
  if (rc != 0) {
    unlink(tmp_path.c_str());  // compiler already printed diagnostics
    return false;
  }
  if (rename(tmp_path.c_str(), cache_path.c_str()) != 0) {
    std::cerr << "xrun: cannot place cached binary: " << std::strerror(errno)
              << "\n";
...
int main(int argc, char** argv) {
  ...
  if (!forge::cache::IsCached(cache_path)) {
    ...
    if (!compile_source(input->abs_path, cache_path)) return kInternalError;
  }

  // Run the cached program: argv[0] = the binary, then the forwarded args.
