...
[[noreturn]] void child_exec_failed(const std::string& prog) {
  std::cerr << "xrun: exec " << prog << " failed: " << std::strerror(errno)
            << "\n";
...
std::optional<CaptureResult> RunCapture(const std::vector<std::string>& argv,
  ...
  while (written < stdin_data.size()) {
    ssize_t n = write(in_pipe[kWriteEnd], stdin_data.data() + written,
...
void RunExec(const std::string& path, const std::vector<std::string>& argv) {
  ...
  std::cerr << "xrun: exec " << path << " failed: " << std::strerror(errno)
            << "\n";
}
